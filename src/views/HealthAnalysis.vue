<template>
  <div class="body">
    ...
    <form>
      <label><p>Enter food items below. (To test, copy items from above!)</p></label> <br />
      <textarea v-model="ingredientInput" rows="1" cols="60"></textarea> <br />
      <button v-on:click="ingredientSubmit">Submit</button>
      <div id="edamam-badge" data-color="white"></div>
    </form>

    <br />
    <div id="nutritionResults"></div> <!-- {{nutritionResults}}-->
  </div>
</template>
...
<script>
  export default {
    ...
    /*watch() {
      nutritionResults: function (val) {
      this.nutritionResults =
    },
    },
    computed: {
      update() {
        return this.nutritionResults; // Declared inside of main.js
      }
    },*/
    methods: {
      /*nutritionResults: function(results) {
        return results;
      },*/
      ingredientSubmit: function(event) {
        event.preventDefault();
        const value = this.ingredientInput;
        if (value === "")
          return;
        console.log(value);
